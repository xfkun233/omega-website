...
<template>
  <section id="gallery" class="mx-auto max-w-6xl px-4 md:px-6 py-16 md:py-24 scroll-mt-[49px]">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <button v-for="(src, i) in images" :key="src" class="relative aspect-video overflow-hidden rounded-lg group"
        @click="open(i)">
    ...
    <Transition name="fade">
      <div v-if="lightbox.open" class="fixed inset-0 z-[60] bg-black/95 flex flex-col items-center justify-center"
        ...
        <div class="relative w-full h-full flex items-center justify-center p-4" @click.self="close">
          ...
          <button
            class="absolute left-4 md:static p-4 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-colors z-50"
            @click.stop="prev">
          ...
          <button
            class="absolute right-4 md:static p-4 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-colors z-50"
            @click.stop="next">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
